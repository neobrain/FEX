...
#   nix profile install .#fex: Install FEX to ~/.nix-profile (with pre-configured x86 RootFS)
...
#   nix flake show:            List all available targets
{
  ...
  outputs =
    ...
    in
    {
      ...
      # FEX builds
      packages.aarch64-linux.fex = pkgs.callPackage ./Data/nix/package.nix { inherit gitRev; };
