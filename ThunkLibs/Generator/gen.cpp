    ...
    // TODO: Move to dedicated function
    {
        ...
        for (auto& thunk : thunks) {
            ...
            for (unsigned param_idx = 0; param_idx != thunk.param_types.size(); ++param_idx) {
                ...
                if (param_type->isPointerType() && param_type->getPointeeType()->isStructureType() &&
                    !is_assumed_compatible && pointee_compat != TypeCompatibility::Full) {
            ...
            for (unsigned param_idx = 0; param_idx != thunk.param_types.size(); ++param_idx) {
                ...
                if (param_type->isPointerType() && param_type->getPointeeType()->isStructureType() &&
                      !is_compatible &&
                      !LookupType(context, param_type->getPointeeType()->getLocallyUnqualifiedSingleStepDesugaredType().getTypePtr()).assumed_compatible &&
